...
function createTooltipNamenProjekte(nameDatensatz) {
    datensatz = dataUnterDimensionenEinzelStandAntrag; /// später 3xif datensatz enstp. para zuweise an antragm zb, tagung
...
createTooltipNamenProjekte("dataUnterDimensionenStandAntrag");
...
series3TechUDim.bullets.push(function () {
    ...
    textTooltip = "[fontSize: 15px]Dimension [fontSize: 14px fontWeight: 500]keine Aussage/keine Relevanz[/] in [fontSize: 14px fontWeight: 500]\n{valueY}%[/][fontSize: 15px] der Vorhaben:"
    tooltipMitProjektNamenSeries3 = textTooltip + "\n[fontSize: 1.2em][fontSize: 0.9em fontStyle: italic]" + tooltipNamenProjekteSeries3[series3TechUDimCounter];
    ...
    circle.states.create("hover", { scale: 1.5, fillOpacity: 0.4 }); //bei mouse over circle, diesen auf das doppelte vergrössern
...
var series2TechUDimCounter = 0; //Dim 1 bis 6 mitzählen für color von label circle in radar column segment
...
series2TechUDim.bullets.push(function () {
    ...
    ///tooltip mit den Projektnamen ergänzen damit in UDim radar sichtbar welche Porjekte ausgeprägt, vorhaden etc sind
    textTooltip = "[fontSize: 15px ]Dimension [fontSize: 15px fontWeight: 500]adressiert[/] in [fontSize: 15px fontWeight: 500 ]{valueY}%[/][fontSize: 15px] der Vorhaben:";
    textTooltipMitProjektNamen = textTooltip + "\n[fontSize: 14px][fontSize: 14px fontStyle: italic]" + tooltipNamenProjekteSeries2[series2TechUDimCounter];
...
var series1TechUDimCounter = 0; //Dim 1 bis 6 mitzählen für color von label circle in radar column segment
...
series1TechUDim.bullets.push(function () {
    ...
    textTooltip = "[fontSize: 15px #fff]Dimension [fontSize: 15px fontWeight: 500]ausgeprägt[/][#fff] in [fontSize: 15px fontWeight: 500 #fff]{valueY}%[/][fontSize: 15px #fff] der Vorhaben:"
    ...
    tooltipMitProjektNamenSeries1 = textTooltip + "\n[fontSize: 14px #fff][fontSize: 14px fontStyle: italic #fff]" + tooltipNamenProjekteSeries1[series1TechUDimCounter];
...
function erstelleRangeTechUDimLabelOnly(indexRange, colorAchseLabel, radiusLabel, labelText, text, startKategorie, endKategorie) { //radius ist abstand des Textlabels in px vom äußeren Kreis
    ...
    //  rangeTechUDimLabelOnly[indexRange].get("axisFill").events.on("pointerover", function (event) { { rangeDataItemTechUDimLabelOnly[indexRange].get("label").showTooltip(); } });
    // rangeTechUDimLabelOnly[indexRange].get("axisFill").events.on("pointerout", function (event) { { rangeDataItemTechUDimLabelOnly[indexRange].get("label").hideTooltip(); } });

    //rangeTechUDimLabelOnly[indexRange].get("axisFill").events.on("pointerover", function (event) { { rangeDataItemTechUDimLabelOnly[indexRange].get("axisFill").showTooltip(); } });
    //rangeTechUDimLabelOnly[indexRange].get("axisFill").events.on("pointerout", function (event) { { rangeDataItemTechUDimLabelOnly[indexRange].get("axisFill").hideTooltip(); } });
...
for (var dim = 1; dim < dataDimensionen.length; dim++) {
    for (var udim = 1; udim <= dataUnterDimensionen.length; udim++) {
        ...
        textUDimTooltip = "[fontSize: 13px fontWeight: 600]" + dataUnterDimensionen[index - 1].nameKurz + "[/]\n[fontSize: 12px fontWeight: 400]" + dataUnterDimensionen[index - 1].textLang + "[/]";
...
for (var index = 0; index < dataDimensionen.length - 1; index++) {
    ...
    textDimTooltip = "[fontSize: 15px fontWeight: 600]" + dataDimensionen[index].nameKurz + "[/]\n[fontSize: 14px fontWeight: 400]" + dataDimensionen[index].textLang + "[/]";
...
var sliderTextStandATechUDim = chartTechUDim.children.push(am5.Label.new(root, { text: "Stand 2021", y: am5.percent(96), x: am5.percent(27), fontSize: "0.9em", fontWeight: 500, fill: ColorBlackYAxisText }));
var sliderTextStandBTechUDim = chartTechUDim.children.push(am5.Label.new(root, { text: "Stand Dezember 2022", y: am5.percent(96), x: am5.percent(49), centerX: am5.percent(50), fontSize: "0.9em", fontWeight: 500, fill: ColorBlackYAxisText }));
var sliderTextStandCTechUDim = chartTechUDim.children.push(am5.Label.new(root, { text: "Stand August 2023", y: am5.percent(96), x: am5.percent(71), centerX: am5.percent(50), fontSize: "0.9em", fontWeight: 500, fill: ColorBlackYAxisText }));
...
function updateValuesDatensatzTechUDim(datensatz) { ///update der value1..3 in series 1..3 bei slider bewegeung
    ...
    am5.array.each(series1TechUDim.dataItems, function (dataItem) {
        var newValue = dataUnterDimensionen[counterUDimDatensatz].value1;
    ...
    if (wirdAdressiertUDimEingeblendet) { /// update values von serie wird adressiert nur wenn in UDim sichtbar
        ...
        am5.array.each(series2TechUDim.dataItems, function (dataItem) {
            var newValue = dataUnterDimensionen[counterUDimDatensatz].value2;
    ...
    if (keineAussageUDimEingeblendet) { /// update values von serie in Ansätzen nur wenn in UDim sichtbar
        ...
        am5.array.each(series3TechUDim.dataItems, function (dataItem) {
            var newValue = dataUnterDimensionen[counterUDimDatensatz].value3;
...
containerTechUDim.hide(); // intial UDim nicht anzeigen, wir bei changeDatensatz gewechselt
